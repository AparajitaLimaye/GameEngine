...
namespace CPI311.GameEngine
{
    public class TerrainRenderer : Component
    {
        ...
        {
            HeightMap = texture;
            this.size = size; // We should also save the value of size somewhere
            CreateHeights(); // heights[] data is crated

            //res = image resolution
            int rows = (int)res.Y + 1; //# of vertexs = rows = res.Y + 1
            int cols = (int)res.X + 1; //# of vertexs = cols = res.X + 1
            ...
            for (int r = 0; r < rows - 1; r++)
                ...
                {
                    //creating each triangle for each square that makes the height map
                    //first triangle
        ...
        {
            Color[] data = new Color[HeightMap.Width * HeightMap.Height]; //create an array of color based on the size of the height map
            HeightMap.GetData<Color>(data);
            heights = new float[HeightMap.Width * HeightMap.Height]; // heigts[] size = height map size
            for (int i = 0; i < heights.Length; i++)
                heights[i] = data[i].G / 255f; //all colors start at the same color?
        }
        ...
        public float GetHeight(Vector2 tex) //tex is UV [0,1]
        {
            ...
            int y = (int)tex.Y; 
            float v = tex.Y - y;
            /// FIX LOOK AT SLIDE 8 OF WEEK 10
            return heights[y * HeightMap.Width + x] * (1 - u) * (1 - v) + //A * (1-u)(1-v)
                heights[y * HeightMap.Width + Math.Min(x+1, HeightMap.Width-1)] * u * (1 - v) + //B * (u)(1-v)
                heights[Math.Min(y+1, HeightMap.Height-1) * HeightMap.Width + x] * (1 - u) * v + //C * (u)(v)
                heights[Math.Min(y+1, HeightMap.Height - 1) * HeightMap.Width + Math.Min(x+1, HeightMap.Width - 1)] * u * v; //D * (1-u)(v)
        }
